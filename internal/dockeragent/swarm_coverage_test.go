	...
	svc := &swarmtypes.Service{
		...
		Endpoint: swarmtypes.Endpoint{
			Ports: []swarmtypes.PortConfig{
				{Name: "http", Protocol: network.TCP, TargetPort: 80, PublishedPort: 8080, PublishMode: swarmtypes.PortConfigPublishModeIngress},
			},
		},
...
func TestCollectSwarmDataFromManager(t *testing.T) {
	agent := &Agent{
		docker: &fakeDockerClient{
			...
			taskListFn: func(_ context.Context, opts taskListOptions) ([]swarmtypes.Task, error) {
...
func TestCollectSwarmData(t *testing.T) {
	...
	t.Run("manager error falls back to containers", func(t *testing.T) {
		agent := &Agent{
			...
			docker: &fakeDockerClient{
				serviceListFn: func(context.Context, serviceListOptions) ([]swarmtypes.Service, error) {
